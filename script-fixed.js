...
window.addEventListener('DOMContentLoaded', function () {
  ...
  if (safetyVideo) {
    ...
    // Also enable button when video is played
    safetyVideo.addEventListener('play', function() {
      safetyVideo.addEventListener('ended', function() {
        videoWatched = true;
        const nextButton = document.querySelector('#next-btn');
        if (nextButton) {
...
function displayCurrentRuleOrQuestion() {
  if (currentRuleIndex < rulesAndQuestions.length) {
    ...
    // Hide all rule/question pages
    const allPages = document.querySelectorAll('#rule-section .rule-page, #rule-section .question-page');
...
function downloadQRCode() {
  ...
  let canvas = null;
  if (resultCanvas && !resultCanvas.closest('.hidden')) {
    canvas = resultCanvas;
  } else if (rule11Canvas && !rule11Canvas.closest('.hidden')) {
    canvas = rule11Canvas;
  }
